...
sl_make_node(sl_parse_state_t* ps, sl_node_type_t type, sl_node_eval_fn_t eval, size_t size)
{
...
sl_make_seq_node(sl_parse_state_t* ps)
{
    sl_node_seq_t* node = (sl_node_seq_t*)
        sl_make_node(ps, SL_NODE_SEQ, (sl_node_eval_fn_t)&sl_eval_seq, sizeof(sl_node_seq_t));
...
{
    MAKE_NODE(SL_NODE_ASSIGN_GLOBAL, NULL, sl_node_assign_var_t, {
        node->lval = lval;
        node->rval = rval;
    });
}
